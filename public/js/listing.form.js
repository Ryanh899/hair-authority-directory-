$(document).ready(function () {
    ...
    $('body').on('click', '#add-faq', function (e) {
        ...
        console.log($(`${questionNumber}faq`))
    ...
    const validateForm = function (formData) {
        ...
        const propNames = Object.getOwnPropertyNames(formData);
        for (var i = 0; i < propNames.length; i++) {
            var propName = propNames[i];
            if (formData[propName] === undefined || formData[propName] === "") {
    ...
    $('body').on('click', '#listing-submit', function (e) {
        ...
        const listingData = {
            ...
            hours: {
                mon: `${formData.get('opening-hours-mon')}-${formData.get('closing-hours-mon')}`,
                tue: `${formData.get('opening-hours-tue')}-${formData.get('closing-hours-tue')}`,
                wed: `${formData.get('opening-hours-wed')}-${formData.get('closing-hours-wed')}`,
                thu: `${formData.get('opening-hours-thu')}-${formData.get('closing-hours-thu')}`,
                fri: `${formData.get('opening-hours-fri')}-${formData.get('closing-hours-fri')}`,
                sat: `${formData.get('opening-hours-sat')}-${formData.get('closing-hours-sat')}`,
                sun: `${formData.get('opening-hours-sun')}-${formData.get('closing-hours-sun')}`
            },
